...
export default function ShopDealsPage() {
  ...
  const copyCode = async (
    ...
    if (success) {
      setCopiedCode(code);
      setShowToast({
        message: `${code} copied! Use at checkout.`,
        type: "success",
      });
      setTimeout(() => {
        setCopiedCode(null);
      ...
      <div className="bg-yellow-500 py-3 px-6 md:px-16 lg:px-20">
        <div className="max-w-7xl mx-auto flex justify-center items-center gap-8 text-sm text-black">
          <span className="font-semibold flex items-center gap-1">
            <Gift className="w-4 h-4" /> {deals.length} Active Deals
          </span>
          <span className="w-1 h-1 bg-black/30 rounded-full" />
          <span className="font-semibold flex items-center gap-1">
            <Percent className="w-4 h-4" /> Up to 20% OFF
          </span>
          <span className="w-1 h-1 bg-black/30 rounded-full" />
          <span className="font-semibold flex items-center gap-1">
          ...
          {discountedProducts.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-2xl border border-gray-100">
              ...
              <p className="text-gray-400">
                No discounted items at the moment.
              </p>
                      ...
                      <button
                        ...
                        className={`absolute bottom-3 right-3 w-8 h-8 rounded-full flex items-center justify-center transition-all ${
                          wishlistState[product.id]
                            ...
                            : "bg-white/90 text-gray-600 hover:bg-yellow-500 hover:text-black"
                        }`}
                      >
                        <Heart
                          className={`w-4 h-4 ${wishlistState[product.id] ? "fill-black" : ""}`}
                        />
                      </button>
                    </div>
                    <div className="p-4">
              ...
              <Link key={product.id} href="/shop" className="group">
                <div className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100">
                  ...
                  <div className="p-3">
                    ...
                    <p className="font-bold text-yellow-600 text-sm mt-1">
                      ${product.price}
                    </p>
                  </div>
                </div>
