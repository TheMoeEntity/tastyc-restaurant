...
import {
  ShoppingCart, Heart, Eye, Star, Flame, Leaf,
  Search, Filter, Grid3x3, List, ChevronRight,
  Tag, Clock, Truck, ShieldCheck, Gift, Sparkles,
  X, Plus, Minus, Check, AlertCircle, Percent,
} from "lucide-react";
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-sm mb-3">
                ...
                <div className="flex gap-2">
                  {product.spicy && <span className="flex items-center gap-1 text-red-500 text-xs"><Flame className="w-3 h-3" /> Spicy</span>}
                  {product.veg && <span className="flex items-center gap-1 text-green-600 text-xs"><Leaf className="w-3 h-3" /> Veg</span>}
                </div>
              </div>
            </div>
            <div className="flex gap-2">
    ...
    <MotionWrapper variant="fade-up">
      <div className="group bg-white rounded-xl shadow-md hover:shadow-xl border border-gray-200 overflow-hidden transition-all duration-300">
        ...
        <div className="p-4">
          ...
          <h3 className="font-bold text-gray-900 group-hover:text-yellow-600 transition-colors mb-1 line-clamp-1">{product.name}</h3>
          ...
          <button onClick={handleAddToCart} className={`w-full py-2 rounded-lg font-semibold transition flex items-center justify-center gap-2 ${isAdded ? "bg-green-500 text-white" : "bg-yellow-500 hover:bg-yellow-400 text-black"}`}>
            {isAdded ? <Check className="w-4 h-4" /> : <ShoppingCart className="w-4 h-4" />}
...
function DealsSection({ deals, onApplyDeal }: { deals: Deal[]; onApplyDeal: (code: string, discount: number, minOrder?: number) => void }) {
  ...
  const copyCode = async (code: string, discount: number, minOrder?: number) => {
    ...
    if (success) {
      ...
      setShowToast({ message: `Please copy this code manually: ${code}`, type: "error" });
                ...
                <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300">
                  <div className="relative h-40 overflow-hidden">
                    ...
                    <div className="absolute bottom-3 left-3">
                      <div className="bg-red-500 text-white text-2xl font-bold px-3 py-1 rounded-lg">{deal.discount}%</div>
                  ...
                  <div className="p-5">
                    ...
                    {deal.minOrder && <p className="text-xs text-gray-400 mb-2">Min. order: ${deal.minOrder}</p>}
...
function WishlistSidebar({ isOpen, onClose, onAddToCart }: { isOpen: boolean; onClose: () => void; onAddToCart: (product: Product) => void }) {
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-out ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex items-center justify-between p-5 border-b border-gray-200">
          <h2 className="text-xl font-bold font-serif text-gray-900 flex items-center gap-2">
            <Heart className="w-5 h-5 text-red-500" /> Wishlist ({wishlistItems.length})
          </h2>
                  ...
                  <div key={item.id} className="flex gap-3 p-3 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex gap-2 mt-2">
                        <button onClick={() => { onAddToCart(product); onClose(); }} className="text-xs bg-yellow-500 text-black px-2 py-1 rounded-lg">Add to Cart</button>
                        <button onClick={() => handleRemove(item.productId)} className="text-xs text-red-500 px-2 py-1">Remove</button>
                      </div>
                    </div>
                  </div>
                );
...
export default function ShopPage() {
  ...
  const handleApplyDeal = (code: string, discount: number, minOrder?: number) => {
  ...
  return (
    <main className="bg-gray-50 min-h-screen">
      ...
      <section className="relative h-[40vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(rgba(0,0,0,0.65), rgba(0,0,0,0.75)), url(/assets/homeImg2.jpg)", backgroundSize: "cover", backgroundPosition: "center" }} />
        <div className="relative z-10 text-center px-6">
          <MotionWrapper variant="fade-up">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold font-serif text-white leading-tight">Our <span className="text-yellow-500">Shop</span></h1>
            <p className="text-white mt-3 text-base sm:text-lg">Discover our curated collection of delicious dishes, from African classics to global favorites.</p>
          </MotionWrapper>
        </div>
      </section>

      {/* Deals Section */}
      {deals.length > 0 && <DealsSection deals={deals} onApplyDeal={handleApplyDeal} />}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div className="relative w-full md:w-96">
              ...
              <input type="text" placeholder="Search products..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:border-yellow-400 transition" />
            </div>
            <div className="flex flex-wrap gap-2 justify-center">
              {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeCategory === cat ? "bg-yellow-500 text-black shadow-md" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>{cat}</button>
              ))}
            </div>
            <div className="flex gap-2">
              <button onClick={() => setShowWishlist(true)} className="relative px-4 py-2 border border-gray-300 rounded-xl hover:border-red-400 transition flex items-center gap-2">
                <Heart className="w-4 h-4" /> Wishlist
              </button>
              <Link href="/cart" className="relative px-4 py-2 border border-gray-300 rounded-xl hover:border-yellow-400 transition flex items-center gap-2">
              ...
              <button onClick={() => setShowFilters(!showFilters)} className="px-4 py-2 border border-gray-300 rounded-xl hover:border-yellow-400 transition"><Filter className="w-4 h-4" /></button>
              <button onClick={() => setViewMode(viewMode === "grid" ? "list" : "grid")} className="px-4 py-2 border border-gray-300 rounded-xl hover:border-yellow-400 transition">{viewMode === "grid" ? <List className="w-4 h-4" /> : <Grid3x3 className="w-4 h-4" />}</button>
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-6 items-end">
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <input type="number" placeholder="Min" value={priceRange[0]} onChange={(e) => setPriceRange([Number(e.target.value), priceRange[1]])} className="w-24 px-2 py-1 border rounded-lg text-sm" />
                    <span>-</span>
                    <input type="number" placeholder="Max" value={priceRange[1]} onChange={(e) => setPriceRange([priceRange[0], Number(e.target.value)])} className="w-24 px-2 py-1 border rounded-lg text-sm" />
                  </div>
                </div>
                <div>
                  <label className="text-sm font-semibold text-gray-700">Sort By</label>
                  <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="w-36 px-2 py-1 border rounded-lg text-sm mt-1">
                ...
                <button onClick={() => { setPriceRange([0, 50]); setSortBy("default"); }} className="px-4 py-1 bg-gray-100 rounded-lg text-sm hover:bg-gray-200">Reset Filters</button>
              </div>
              <div className="flex flex-wrap gap-3 mt-4">
                {["Spicy", "Vegetarian", "Popular"].map(filter => (
                  <button key={filter} className="px-3 py-1.5 text-sm bg-gray-50 border border-gray-200 rounded-full hover:border-yellow-400 transition">{filter}</button>
                ))}
              </div>
            </MotionWrapper>
          )}
      ...
      <section className="py-16 px-6 md:px-16 lg:px-20">
        <div className="max-w-7xl mx-auto">
          ...
          {products.length === 0 ? (
            <div className="text-center py-12"><p className="text-gray-400">No products found matching your criteria.</p></div>
          ) : (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="text-center">
                <div className="w-12 h-12 bg-yellow-100 rounded-full flex items-center justify-center mx-auto mb-3"><Icon className="w-6 h-6 text-yellow-600" /></div>
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center px-6">
          <MotionWrapper variant="fade-up">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              <Link href="/menu" className="px-8 py-3 bg-yellow-500 hover:bg-yellow-400 text-black font-bold rounded-xl transition">Full Menu</Link>
              <Link href="/cart" className="px-8 py-3 bg-transparent border-2 border-white hover:bg-white/10 text-white font-bold rounded-xl transition">View Cart</Link>
            </div>
          </MotionWrapper>
        </div>
      </section>
      ...
      {/* Wishlist Sidebar */}
      <WishlistSidebar isOpen={showWishlist} onClose={() => setShowWishlist(false)} onAddToCart={handleAddToCart} />
    </main>
  );
}