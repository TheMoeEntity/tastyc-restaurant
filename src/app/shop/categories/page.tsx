...
export default function ShopCategoriesPage() {
  ...
  const filteredCategories: CategoryItem[] = categoryList.filter((cat: CategoryItem) => 
        ...
        <div className="max-w-4xl mx-auto text-center">
          <MotionWrapper variant="fade-up">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <p className="text-sm font-bold uppercase tracking-widest text-yellow-500">Shop by Category</p>
              <div className="h-0.5 w-6 bg-yellow-500" />
            </div>
            ...
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Find exactly what you're looking for by exploring our product collections
            </p>
          </MotionWrapper>
        </div>
      </section>
          ...
          {filteredCategories.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCategories.map((category: CategoryItem, idx: number) => {
                ...
                const bgImage = categoryBgImages[category.name] || "/assets/homeImg1.jpg";
                
                return (
                  <MotionWrapper key={category.slug} variant="fade-up" delay={idx * 100}>
                    <Link 
                      href={category.name === "All" ? "/shop" : `/shop?category=${category.slug}`}
                        ...
                        <div className="relative h-full flex flex-col justify-end p-6 z-10">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="bg-black/50 backdrop-blur-sm rounded-full px-3 py-1">
                              ...
                              <span className="text-white/70 text-sm ml-1">items</span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between pt-3 border-t border-white/20">
                            ...
                            <span className="text-yellow-400 font-medium group-hover:translate-x-2 transition-transform duration-300 flex items-center gap-1">
                              Shop Now <ArrowLeft className="w-4 h-4 rotate-180" />
                            </span>
                          </div>
                        </div>
                      </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10">
            <div className="flex items-center justify-center gap-3 mb-3">
              <div className="h-0.5 w-6 bg-yellow-500" />
              <p className="text-sm font-bold uppercase tracking-widest text-yellow-500">Popular Picks</p>
              <div className="h-0.5 w-6 bg-yellow-500" />
            </div>
            <h2 className="text-2xl md:text-3xl font-bold font-serif text-gray-900">
              Most Loved Products
            </h2>
            <p className="text-gray-500 mt-2">What our customers are buying right now</p>
          </div>
