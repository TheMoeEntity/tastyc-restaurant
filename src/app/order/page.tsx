        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div key={step.key} className="relative flex flex-col items-center">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center z-10 transition-all duration-300 ${isCompleted ? "bg-yellow-500 text-black" : "bg-gray-200 text-gray-400"} ${isCurrent ? "ring-4 ring-yellow-200 scale-110" : ""}`}
              >
                <Icon className="w-5 h-5" />
              </div>
              <span
                className={`text-xs mt-2 font-medium hidden sm:block ${isCompleted ? "text-gray-700" : "text-gray-400"}`}
              >
                {step.label}
              </span>
            </div>
...
function OrderDetailModal({
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ${isVisible ? "bg-black/60 backdrop-blur-sm" : "bg-black/0 pointer-events-none"}`}
      onClick={handleClose}
    >
      <div
        className={`bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl transition-all duration-300 ${isVisible ? "scale-100 opacity-100" : "scale-95 opacity-0"}`}
        ...
        <div className="p-4 sm:p-5 space-y-5 sm:space-y-6">
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              <span className="text-sm font-semibold">{statusConf.label}</span>
            </div>
            <div
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-2">
              <p className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-sm">
                <span className="font-medium text-gray-700 sm:w-24">Name:</span>
