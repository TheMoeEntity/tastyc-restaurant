...
export default function UserReservationsPage() {
  ...
  const runFetch = () =>
    fetch(`${API}/api/reservations`, { credentials: "include" })
      .then(async (r) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-white/40 text-sm">Your upcoming and past table reservations</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 py-12 text-center">
          ...
          <button
            onClick={() => { setLoading(true); runFetch(); }}
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-white/40 text-xs">Party of {rv.partySize}</p>
                  {rv.tableNumber && (
                    <p className="text-white/30 text-xs">Table #{rv.tableNumber}</p>
                  )}
                  {rv.notes && (
