...
export default function AdminReviewsPage() {
    ...
    const handleApprove = async (id: string) => {
        setApprovingId(id);
        try {
            const res = await apiFetch<any>(`/api/reviews/${id}/approve`, {
                method: "PATCH",
            });
    ...
    const handleDelete = async (id: string) => {
        ...
        setDeletingId(id);
        try {
            const res = await apiFetch<any>(`/api/reviews/${id}`, {
                method: "DELETE",
            });
            if (!res.success) throw new Error(res.message);
            ...
            <div className="flex gap-1 bg-white/5 border border-white/10 rounded-xl p-1 w-fit">
                {(["pending", "approved"] as Tab[]).map((t) => (
                    <button
                        key={t}
                        onClick={() => setTab(t)}
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-semibold capitalize transition ${tab === t
