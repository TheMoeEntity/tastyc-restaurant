...
export default function MenuItemClient({ slug }: { slug: string }) {
    ...
    // ── Fetch item ──────────────────────────────────────────────
    useEffect(() => {
        apiFetch<any>(`/api/menu/slug/${slug}`)
            .then((r) => {
                if (!r.success) throw new Error(r.message);
                setItem(r.data.item ?? r.data);
            })
            .catch((err) =>
    ...
    const fetchReviews = (page = 1) => {
        ...
        apiFetch<any>(`/api/reviews?menuItemId=${item.id}&page=${page}&limit=5`)
            .then((r) => {
                if (!r.success) throw new Error(r.message);
    ...
    useEffect(() => {
        ...
        apiFetch<any>(
            ...
            .then((r) => {
                ...
                setSimilarItems(all.filter((i: MenuItem) => i.slug !== slug).slice(0, 3));
            })
            .catch(() => { });
    }, [item]);
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            ...
            toast.success(
                "Review submitted! It will appear after manager approval.",
            );
            setRating(0);
            setComment("");
        } catch (err) {
    ...
    const isVeg =
        item.tags.includes("vegan") || item.tags.includes("vegetarian");
        ...
        <main className="bg-gray-50 min-h-screen">
            ...
            <div className="max-w-6xl mx-auto px-4 pt-24 pb-4">
                <nav className="flex items-center gap-2 text-sm text-gray-400">
                    ...
                    <span>/</span>
                    <Link href="/menu" className="hover:text-gray-600 transition">Menu</Link>
                    <span>/</span>
                    <Link
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-xl border text-sm font-semibold transition ${!selectedVariant
                                    ...
                                    {item.variants.map((v) => (
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded-xl border text-sm font-semibold transition ${selectedVariant?.id === v.id
                                                ? "bg-yellow-500 border-yellow-500 text-black"
                                                : !v.isAvailable
                                        ...
                                        >
                                            ...
                                            {v.priceDelta < 0 && ` -₦${Math.abs(v.priceDelta).toLocaleString()}`}
                                        </button>
                                    ))}
                                </div>
                            </div>
